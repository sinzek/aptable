...
export async function GET(request: Request) {
    try {
        ...
        if (!email) {
            return NextResponse.json({ exists: false, message: 'Email parameter is required' }, { status: 400 });
        }
        ...
        return NextResponse.json({ exists: false, message: 'Server error checking email' }, { status: 500 });
    }
}